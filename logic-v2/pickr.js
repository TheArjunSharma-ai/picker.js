(function (window) {
        ...
        return `
            <div class="tp-header">
                ...
                <div class="tp-display-row">
                    <span class="tp-display-part" data-part="hour">
                    <input type="text" min="${minHour}" value='${hour2Digit ? pad(hour) : hour}' max="${maxHour}"/>
                    </span>
                    <span class="tp-display-colon">:</span>
                    <span class="tp-display-part tp-dim" data-part="min">
                    <input type="text" min="${min2Digit ? '00' : '0'}" value='${min2Digit ? pad(minute) : minute}' max="59"/>
                    </span>
                    ${timeToggleHTML(is12)}
                </div>
                ${innerHeader}
    ...
    const updateClockDisplay = (drop, mode, hour, minute, isAM, is12) => {
        drop.querySelector('[data-part="hour"]').textContent = pad(hour);
        drop.querySelector('[data-part="min"]').textContent = pad(minute);
    ...
    function timer(options) {
      ...
      ${footerHTML('Cancel', 'Set Time')}`;
        const render = () => {
            timeHeaderHTML(HEADER, hour, minute, is12, '', cfg.format)
            drop.querySelector('[data-val="hour"]').textContent = IsDoubleDigit(cfg.format, 'hh') ? pad(hour) : hour;
            drop.querySelector('[data-part="min"]').textContent = IsDoubleDigit(cfg.format, 'mm') ? pad(minute) : minute;
